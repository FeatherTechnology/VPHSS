...
if ($tempStudentType == "1" || $tempStudentType == "2") {
    $student_type_cndtn = "(fm.student_type = '$tempStudentType' || fm.student_type = '4')";
} else {
    $student_type_cndtn = "(fm.student_type = '$tempStudentType')";
}
...
if($CheckReceiptQry->rowCount() > 0){
    ...
    $feeDetailsQry = $connect->query("SELECT fm.fees_id, fm.academic_year, gcf.*  FROM `fees_master` fm JOIN group_course_fee gcf ON fm.fees_id = gcf.fee_master_id where fm.academic_year = '$academicYear' && fm.medium = '$tempMedium' && $student_type_cndtn && fm.standard = '$standardId' && gcf.status ='1' ");
